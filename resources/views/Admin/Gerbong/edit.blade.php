...
@section('judul', 'Edit Gerbong - Manajemen Tiket Kereta')
...
<main class="container mx-auto px-4 py-8 max-w-7xl">
    ...
    <div class="mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="text-gray-600">Perbarui data gerbong, <span class="font-semibold">{{ now()->format('d F Y') }}</span></p>
            </div>
    ...
    <div class="bg-white rounded-xl p-6 shadow-lg hover:bg-gray-50 transition">
        <form action="{{ route('admin.gerbong.update', $gerbong) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="kereta_id" id="kereta_id" class="w-full border border-gray-200 rounded-lg px-4 py-2 focus:outline-none focus:border-indigo-500">
                        ...
                        @foreach ($keretas as $kereta)
                            <option value="{{ $kereta->id }}" {{ $gerbong->kereta_id == $kereta->id ? 'selected' : '' }}>{{ $kereta->nama }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="kode_gerbong" id="kode_gerbong" value="{{ old('kode_gerbong', $gerbong->kode_gerbong) }}" class="w-full border border-gray-200 rounded-lg px-4 py-2 focus:outline-none focus:border-indigo-500">
                ...
                <div>
                    ...
                    <input type="number" name="jumlah_kursi" id="jumlah_kursi" value="{{ old('jumlah_kursi', $gerbong->jumlah_kursi) }}" min="1" class="w-full border border-gray-200 rounded-lg px-4 py-2 focus:outline-none focus:border-indigo-500">
            ...
            <div class="mt-6 flex gap-3">
                ...
                <button type="submit" class="bg-indigo-600 text-white px-6 py-3 rounded-lg hover:bg-indigo-700 transition transform hover:scale-105">Perbarui Gerbong</button>
            </div>
        </form>
    </div>
</main>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (form) {
            form.addEventListener('submit', () => {
                showNotification('Memperbarui gerbong', 'info');
            });
        }
