    ...
    <div class="mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="text-gray-600">Perbarui pemesanan, <span class="font-semibold">{{ now()->format('d F Y') }}</span></p>
            </div>
        ...
        <form action="{{ route('pemesanan.update', $pemesanan) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="jadwal_id" id="jadwal_id" class="w-full border border-gray-200 rounded-lg px-4 py-2 focus:outline-none focus:border-indigo-500">
                        ...
                        @foreach ($jadwals as $jadwal)
                            <option value="{{ $jadwal->id }}" {{ $pemesanan->jadwal_id == $jadwal->id ? 'selected' : '' }}>{{ $jadwal->kereta->nama }}: {{ $jadwal->stasiun_awal }} - {{ $jadwal->stasiun_akhir }} ({{ $jadwal->tanggal->format('d/m/Y') }})</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="jumlah_penumpang" id="jumlah_penumpang" value="{{ old('jumlah_penumpang', $pemesanan->jumlah_penumpang) }}" min="1" class="w-full border border-gray-200 rounded-lg px-4 py-2 focus:outline-none focus:border-indigo-500">
                ...
                <div>
                    ...
                    <select name="status" id="status" class="w-full border border-gray-200 rounded-lg px-4 py-2 focus:outline-none focus:border-indigo-500">
                        ...
                        <option value="paid" {{ $pemesanan->status == 'paid' ? 'selected' : '' }}>Paid</option>
                                ...
                                <div class="flex-1">
                                    <select name="penumpang_ids[]" class="w-full border border-gray-200 rounded-lg px-4 py-2 focus:outline-none focus:border-indigo-500">
                                        ...
                                        @foreach ($penumpangs as $penumpang)
                                            <option value="{{ $penumpang->id }}" {{ $detail->penumpang_id == $penumpang->id ? 'selected' : '' }}>{{ $penumpang->nama }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="flex-1">
                                    <select name="gerbong_ids[]" class="w-full border border-gray-200 rounded-lg px-4 py-2 focus:outline-none focus:border-indigo-500">
                                        ...
                                        @foreach ($gerbongs as $gerbong)
                                            <option value="{{ $gerbong->id }}" {{ $detail->gerbong_id == $gerbong->id ? 'selected' : '' }}>{{ $gerbong->kode_gerbong }}</option>
                                        @endforeach
                                    </select>
                                </div>
            ...
            <div class="mt-6 flex gap-3">
                ...
                <button type="submit" class="bg-indigo-600 text-white px-6 py-3 rounded-lg hover:bg-indigo-700 transition transform hover:scale-105">Perbarui Pemesanan</button>
            </div>
        </form>
    </div>
</main>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (form) {
            form.addEventListener('submit', () => {
                showNotification('Memperbarui pemesanan', 'info');
            });
        }
