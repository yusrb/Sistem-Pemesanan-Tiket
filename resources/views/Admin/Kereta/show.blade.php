...
@section('judul', 'Detail Kereta - Manajemen Tiket Kereta')
...
<main class="container mx-auto px-4 py-8 max-w-7xl">
    ...
    <div class="mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="text-gray-600">Informasi kereta {{ $kereta->nama }}, <span class="font-semibold">{{ now()->format('d F Y') }}</span></p>
            </div>
    ...
    <div class="bg-white rounded-xl p-6 shadow-lg hover:bg-gray-50 transition">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <p class="text-sm font-semibold text-gray-600">Nama Kereta</p>
                <p class="text-lg text-gray-900">{{ $kereta->nama }}</p>
            </div>
            <div>
                <p class="text-sm font-semibold text-gray-600">Jumlah Gerbong</p>
                <p class="text-lg text-gray-900">{{ $kereta->jumlah_gerbong }}</p>
            </div>
        </div>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (deleteForm) {
            deleteForm.addEventListener('submit', () => {
                showNotification('Menghapus kereta', 'info');
            });
        }
