...
@section('judul', 'Edit Kereta - Manajemen Tiket Kereta')
...
<main class="container mx-auto px-4 py-8 max-w-7xl">
    ...
    <div class="mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="text-gray-600">Perbarui data kereta, <span class="font-semibold">{{ now()->format('d F Y') }}</span></p>
            </div>
    ...
    <div class="bg-white rounded-xl p-6 shadow-lg hover:bg-gray-50 transition">
        <form action="{{ route('admin.kereta.update', $kereta) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 gap-6">
                <div>
                    <label for="nama" class="block text-sm font-semibold text-gray-600 mb-2">Nama Kereta</label>
                    <input type="text" name="nama" id="nama" value="{{ old('nama', $kereta->nama) }}" class="w-full border border-gray-200 rounded-lg px-4 py-2 focus:outline-none focus:border-indigo-500">
            ...
            <div class="mt-6 flex gap-3">
                ...
                <button type="submit" class="bg-indigo-600 text-white px-6 py-3 rounded-lg hover:bg-indigo-700 transition transform hover:scale-105">Perbarui Kereta</button>
            </div>
        </form>
    </div>
</main>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (form) {
            form.addEventListener('submit', () => {
                showNotification('Memperbarui kereta', 'info');
            });
        }
