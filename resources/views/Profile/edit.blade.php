...
@section('judul', 'Edit Profil - Manajemen Tiket Kereta')
    ...
    <div class="mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="text-gray-600">Perbarui informasi profil Anda, <span class="font-semibold">{{ now()->format('d F Y') }}</span></p>
            </div>
    ...
    <div class="bg-white rounded-xl p-6 shadow-lg hover:bg-gray-50 transition">
        <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label for="password_confirmation" class="block text-sm font-semibold text-gray-600 mb-2">Konfirmasi Kata Sandi</label>
                    <input type="password" name="password_confirmation" id="password_confirmation" class="w-full border border-gray-200 rounded-lg px-4 py-2 focus:outline-none focus:border-indigo-500">
                </div>
            </div>
            <div class="mt-6 flex gap-3">
                <a href="{{ url()->previous() }}" class="bg-gray-600 text-white px-6 py-3 rounded-lg hover:bg-gray-700 transition inline-block text-center">Kembali</a>
                <button type="submit" class="bg-indigo-600 text-white px-6 py-3 rounded-lg hover:bg-indigo-700 transition transform hover:scale-105">Perbarui Profil</button>
            </div>
        </form>
    </div>
</main>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (form) {
            form.addEventListener('submit', () => {
                showNotification('Memperbarui profil', 'info');
            });
        }
