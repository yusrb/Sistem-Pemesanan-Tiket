...
@section('judul', 'Edit Penumpang - Manajemen Tiket Kereta')
...
<main class="container mx-auto px-4 py-8 max-w-7xl">
    <div class="mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="text-gray-600">Perbarui data penumpang, <span class="font-semibold">{{ now()->format('d F Y') }}</span></p>
            </div>
    ...
    <div class="bg-white rounded-xl p-6 shadow-lg hover:bg-gray-50 transition">
        <form action="{{ route('penumpang.update', $penumpang) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="bg-white rounded-xl p-6 shadow-lg hover:bg-gray-50 transition">
        <form action="{{ route('penumpang.update', $penumpang) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="nama" id="nama" value="{{ old('nama', $penumpang->nama) }}" class="w-full border {{ $errors->has('nama') ? 'border-red-500' : 'border-gray-200' }} rounded-lg px-4 py-2 focus:outline-none focus:border-indigo-500" required>
                    @error('nama')
                ...
                <div>
                    ...
                    <input type="text" name="nik" id="nik" value="{{ old('nik', $penumpang->nik) }}" class="w-full border {{ $errors->has('nik') ? 'border-red-500' : 'border-gray-200' }} rounded-lg px-4 py-2 focus:outline-none focus:border-indigo-500" maxlength="16" required>
                    @error('nik')
                        <div class="flex items-center gap-1 mt-1 text-red-600 text-xs">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"/>
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" 
                        ...
                        class="w-full border {{ $errors->has('email') ? 'border-red-500' : 'border-gray-200' }} rounded-lg px-4 py-2 focus:outline-none focus:border-indigo-500" required>
                    @error('email')
                        <div class="flex items-center gap-1 mt-1 text-red-600 text-xs">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"/>
                ...
                <div>
                    ...
                    <input type="text" name="no_telepon" id="no_telepon" value="{{ old('no_telepon', $penumpang->no_telepon) }}" class="w-full border {{ $errors->has('no_telepon') ? 'border-red-500' : 'border-gray-200' }} rounded-lg px-4 py-2 focus:outline-none focus:border-indigo-500" maxlength="15">
                    @error('no_telepon')
                        <div class="flex items-center gap-1 mt-1 text-red-600 text-xs">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"/>
            ...
            <div class="mt-6 flex gap-3">
                ...
                <button type="submit" class="bg-indigo-600 text-white px-6 py-3 rounded-lg hover:bg-indigo-700 transition transform hover:scale-105">Perbarui</button>
            </div>
        </form>
    </div>
        </form>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (form) {
        form.addEventListener('submit', () => {
            showNotification('Memperbarui penumpang, Tuan', 'info');
        });
    }
