...
<main class="container mx-auto px-4 py-8 max-w-7xl">
    ...
    <div class="bg-white rounded-xl p-6 shadow-lg mb-6 hover:bg-gray-50 transition">
        <form method="GET" action="{{ route('petugas.pemesanan.index') }}" class="flex flex-col sm:flex-row sm:items-center gap-4 flex-1">
            <div class="flex-1">
                <input type="text" name="search" value="{{ $search }}" placeholder="Cari nama pengguna, stasiun, atau kode tiket..." class="w-full border border-gray-200 rounded-lg px-4 py-2 focus:outline-none focus:border-indigo-500">
            </div>
            <div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($pemesanans as $pemesanan)
                        <tr class="hover:bg-indigo-50 transition cursor-pointer" onclick="window.location='{{ route('petugas.pemesanan.show', $pemesanan) }}'">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                @if ($pemesanan->status === 'pending')
                                    <a href="{{ route('petugas.pemesanan.show', $pemesanan) }}" onclick="event.stopPropagation()" class="text-indigo-600 hover:text-indigo-800 hover:underline">Validasi</a>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-4 text-center text-sm text-gray-600">Tidak ada pemesanan.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
